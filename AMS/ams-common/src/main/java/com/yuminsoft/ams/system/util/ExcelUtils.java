...
public class ExcelUtils{
	 ...
	 * @param dataset  数据内容
	 * @param out      文件输出流
	 * @param pattern  设定时间类型输出格式。默认为"yyy-MM-dd"
	 */
	public static <T> boolean exportExcel(String fileName, String[] headers, Collection<T> dataset, HttpServletRequest request, HttpServletResponse response, String pattern) throws Exception {
		...
		while (it.hasNext()) {
			...
			for (Field field : fields) {
				...
				//序列化ID不需要输出
				if (AmsConstants.SERIAL_VERSION_UID.equals(fieldName)) {
					continue;
				}
	...
	public static <T> boolean exportExcel(String fileName,String[] properties, Collection<T> dataset, HttpServletRequest request, HttpServletResponse response, String pattern, Class clazz) throws Exception {
		...
		XSSFSheet sheet = workbook.createSheet(fileName);    	// 生成一个表格
		sheet.setDefaultColumnWidth((short) 20);            	// 设置表格默认列宽度为20个字节

		XSSFCellStyle style = workbook.createCellStyle();    	// 生成一个样式

		style.setAlignment(XSSFCellStyle.ALIGN_CENTER);       	 // 设置样式
		XSSFCell cell = null;

		XSSFRow row = sheet.createRow(0);      // 设置表格标题行
		...
		while (it.hasNext()) {
			...
			for (String property : properties) {
				...
				if (value instanceof Date) {	// 判断值的类型后进行强制类型转换
					Date date = (Date) value;
					SimpleDateFormat format = new SimpleDateFormat(pattern);
					if (value != null) {
						cell.setCellValue(format.format(date));
