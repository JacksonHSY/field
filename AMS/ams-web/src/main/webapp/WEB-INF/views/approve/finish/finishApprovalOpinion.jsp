		...
		<div id="finishApprovalOpinion_approveInfo_div" style="margin-top: 0px; width: 100%; border-bottom: 2px solid #95b8e7;">
			...
			<div class="easyui-panel" title="申请信息">
				<table class="table_ui W100">
					...
					<tr>
						<th width="12%">申请产品:</th>
						<td width="20%">${applyBasiceInfo.initProductName}</td>
						<th width="12%">申请期限:<input type="hidden" value="${applyBasiceInfo.loanNo}" id="loanNo"> <input type="hidden" id="apply_term_id" value="${applyBasiceInfo.applyTerm}">
						</th>
						<td width="20%">${applyBasiceInfo.applyTerm}</td>
						<th width="12%">申请额度:<input type="hidden" id="applyLmt" value="${applyBasiceInfo.applyLmt}"></th>
		...
		<div class="easyui-panel" title="资料核对">
				...
				<table class="table_ui W100">
					<tr>
						...
						<td width="20%" ><fmt:formatNumber type="number" value="${applyInfo.basicInfoVO.personInfoVO.monthMaxRepay}"  maxFractionDigits="0" />元</td>
						<th width="12%">客户工作类型:</th>
						<td width="20%">${applyInfo.basicInfoVO.workInfoVO.cusWorkType}</td>
						<th width="12%">发薪方式:</th>
	...
	<div class="easyui-panel" id="finish_approval_assetsInfo_div" title="产品信息" data-name="${applyInfo.applyInfoVO.productCd}">
		...
		<table class="table_list W100 display_noH" id="MASTERLOAN_A">
			...
			<tr>
				...
				<td>淘气值:</td>
				<td colspan="3"><fmt:formatNumber type="number" value="${applyInfo.assetsInfoVO.masterLoanInfoAVO.naughtyValue}" maxFractionDigits="0" /></td>
			</tr>
			<tr>
				<td>3个月内收货地址</td>
					...
					<c:if test="${empty currentApprovalHistory}">
						...
						<table class="table_ui W100">
							...
							<tr>
								...
								<td width="20%">${applyBasiceInfo.applyTerm}</td>
								<th width="12%">审批产品:</th>
								<td><input type="text" class="input" name="approvalProductCd" value="${applyInfo.applyInfoVO.productCd}"></td>
							</tr>
							<tr>
								<th class="requiredFontWeight">核实收入:</th>
								<td><input type="text" id="approvalCheckIncome" data-options="groupSeparator:','" name="approvalCheckIncome" class="input" value="${item.approvalCheckIncome}"></td>
								<th class="requiredFontWeight">终审审批额度:<input id="finishApprovalOpinion_applyMoney_hidden" type="hidden" placeholder="申请额度用于控制审批额度上下限" value="${applyInfo.applyInfoVO.applyLmt}"></th>
								<td ><input type="text" id="approvalLimit" data-options="groupSeparator:','"  name="approvalLimit" class="input"></td>
								<th class="requiredFontWeight">审批期限:</th>
								<td><input type="text" class="input" name="approvalTerm"></td>
							</tr>
							<tr>
								<th>月还款额:<input type="hidden" id="approvalMonthPay" name="approvalMonthPay"></th>
					...
					<c:if test="${not empty currentApprovalHistory}">
						...
						<c:if test="${item.approvalPerson == currentApprovalHistory.approvalPerson}">
							...
							<table class="table_ui W100">
								...
								<tr>
									<th class="requiredFontWeight">核实收入:</th>
									<td><input type="text" id="approvalCheckIncome" name="approvalCheckIncome" data-options="groupSeparator:','" class="input" value="${item.approvalCheckIncome}"></td>
									<th class="requiredFontWeight">终审审批额度:<input id="finishApprovalOpinion_applyMoney_hidden" type="hidden" placeholder="申请额度用于控制审批额度上下限" value="${applyInfo.applyInfoVO.applyLmt}"></th>
									<td><input type="text" id="approvalLimit" name="approvalLimit" data-options="groupSeparator:','"  class="input" value="${item.approvalLimit}"></td>
									<th class="requiredFontWeight">审批期限:</th>
						...
						<c:if test="${item.approvalPerson != currentApprovalHistory.approvalPerson}">
							...
							<table class="table_ui W100">
								...
								<tr>
									...
									<td><input type="text" id="approvalCheckIncome" name="approvalCheckIncome" data-options="groupSeparator:','" class="input" value="${item.approvalCheckIncome}"></td>
									<th class="requiredFontWeight">终审审批额度:<input id="finishApprovalOpinion_applyMoney_hidden" type="hidden" placeholder="申请额度用于控制审批额度上下限" value="${applyInfo.applyInfoVO.applyLmt}"></th>
									<td><input type="text" id="approvalLimit" name="approvalLimit" data-options="groupSeparator:','"  class="input" ></td>
									<th class="requiredFontWeight">审批期限:</th>
									<td><input type="text" class="input" name="approvalTerm"></td>
								</tr>
								<tr>
