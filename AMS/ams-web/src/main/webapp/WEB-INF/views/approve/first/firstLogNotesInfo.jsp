	...
	<table class="table_list W100" style="table-layout: fixed;">
		...
		<c:if test="${not empty loanLogList}">
			<c:forEach var="item" items="${loanLogList}">
				<tr>
					<td>
						<div style="word-wrap: break-word;">${item.operationModule}</div>
					</td>
					<td>
						<div style="word-wrap: break-word;">${item.operationType}</div>
					</td>
					<td>
						<div style="word-wrap: break-word;">${item.operator}</div>
					</td>
					<td>
						<div style="word-wrap: break-word;">
							<fmt:formatDate value="${item.operationTime}" pattern="yyyy-MM-dd HH:mm:ss" />
						</div>
					</td>
					<td>
						<div style="word-wrap: break-word;">${item.remark}</div>
					</td>
					<td>
						<div style="word-wrap: break-word;">${item.firstLevleReasons}${item.twoLevleReasons}</div>
					</td>
				</tr>
			</c:forEach>
		</c:if>
