			...
			<div class="easyui-panel" title="申请信息">
				<table class="table_ui W100">
					<tr>
						...
						<th>身份证号码:</th>
						<td>{{ applyInfo.baseInfo.idNo }}</td>
					</tr>
					<tr>
						<th width="16%">申请产品:</th>
						<td width="16%" ms-if="'证大前前'!=applyInfo.baseInfo.initProductName">{{ applyInfo.baseInfo.initProductName }}</td>
						<th width="16%">申请期限:</th>
						<td width="16%">{{ applyInfo.baseInfo.applyTerm }}<input ms-duplex="loanNo" id="loanNo" type="hidden"></td>
						<th width="16%">申请额度:</th>
						<td class="numFormat">{{ numberRound(applyInfo.baseInfo.applyLmt) }}</td>
					</tr>
				</table>
			</div>
		</div>
			...
			<form id="finishApprovalOpinion_approvalInfo_from">
				...
				<table class="table_ui W100">
					...
					<tr>
						...
						<th>审批产品:<c:if test="${stat.last}"><input id="money_selectAssetType_hidden"  type="hidden" name="assetType" placeholder="用于记录审批勾选资产类型" value="${item.assetType}"></c:if></th>
