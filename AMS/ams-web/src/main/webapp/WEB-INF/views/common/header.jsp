...
<link rel="stylesheet" type="text/css" href="${ctx}/resources/css/ams.css"/>
<style>
    .header{
        font: 12px/1.6em "微软雅黑", Verdana, Arial, sans-serif;
    }
...
<script>
    ...
    function logout() {
        $.messager.confirm('提示', '确定要退出?', function (r) {
            window.removeEventListener("beforeunload", cleanOpenedDialog);
            if (r) {
                jDialog.closeAll();        // 关闭所有未关闭的窗口
                window.location.href = basePath + '/logout';    // 跳转到登录页面
            }
        });
    }
...
//                 $('#empMessage').attr('src', basePath + '/message/empMessageView?account=' + account);
    ...
    function socket(url){
        ...
        messageWebSocket.onmessage = function (e) {
            ...
            if (result.success) {
                if (result.data > 0) {
                    $("#unreadAccount").text(result.data > 99 ? "99+":result.data);
					...
					if (responseText) {
						if(responseText.indexOf("login_status") > 0){
                            ...
                            if(response.login_status == 300){
                                window.location.href = basePath + "/";//刷新当前页
                                return true;
                            }
                        }else{
...
<div class="header">
    <div style="text-align:right; color:#ffffff; padding:8px 0px">
        ...
        <span class="logo1" style="display:block;float: left;">
            <img style="width:100%;height:100%;" src="${ctx}/resources/images/logo.png"></span>
        </div>
        ...
        <a id="unreadMessage" href="javascript:;" class="easyui-linkbutton" data-options="plain:true,iconCls:'fa fa-comments'" onclick="showMsg()">消息中心
            <div id="unreadCountDiv" style="display: none;width:20px;height:16px;line-height:14px;border-radius:8px;background:#f00;color:#fff;text-align: center;"><span id="unreadAccount"></span></div>
        </a> │
