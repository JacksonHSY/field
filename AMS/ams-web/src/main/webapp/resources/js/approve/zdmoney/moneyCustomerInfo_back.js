...
avalon.ready(function () {
    ...
    post(ctx.rootPath() + "/bmsBasiceInfo/getProvinceCorrespond", null, null, function (data) {
        ...
        // 住宅地址
        inintArea(data, "#moneyCustomerInfo_family_province_combobox", "#moneyCustomerInfo_family_city_combobox", "#moneyCustomerInfo_family_country_combobox");
        // 单位地址
    ...
    $("[name='estateSameRegistered']","#money_customer_ESTATE_div").on("change", function (e) {
        ...
        if (yesOrNo == 'Y') {
            ...
            if (isEmpty(homeProv) || isEmpty(homeCity) || isEmpty(homeCoun) || isEmpty(homeAddr)) {
                $.info("提示", "住宅地址不完整，请先填写完整住宅地址!", "warning");
...
function save(loanBaseId, loanNo,productCd, version) {
    ...
    if (!firstCustomerMarkSubmit) {
        ...
        // 验证保险信息
        var policyInfoValidate = true;
        $("#money_customer_POLICY_div").find("form").each(function(ind,formObj){
            if (!$(formObj).form("validate")) {
                policyInfoValidate = false;
                focusTab('assets');
                $(formObj).form("validate");
                return false;
            }
        });
        if (!policyInfoValidate) {// 验证保险信息
            return false;
        }

        // 车辆信息
        if (!$("#money_customerCar_form").form("validate")) {
            focusTab('assets');
            $("#money_customerCar_form").form("validate");
            return false;
        }

        // 公积金信息
        if (!$("#money_customerProvident_form").form("validate")) {
            focusTab('assets');
            $("#money_customerProvident_form").form("validate");
            return false;
        }

        // 社保信息
        if (!$("#money_customerSOCIAL_INSURANCE_form").form("validate")) {
            focusTab('assets');
            $("#money_customerSOCIAL_INSURANCE_form").form("validate");
            return false;
        }

        // 卡友贷信息
        if (!$("#money_customerCardLoan_form").form("validate")) {
            focusTab('assets');
            $("#money_customerCardLoan_form").form("validate");
            return false;
        }

        // 房产信息
        if (!$("#money_customerHome_form").form("validate")) {
            focusTab('assets');
            $("#money_customerHome_form").form("validate");
            return false;
        }

        // 淘宝账号
        if (!$("#money_customerMerchant_form").form("validate")) {
            focusTab('assets');
            $("#money_customerMerchant_form").form("validate");
            return false;
        }

        // 学历信息
        if (!$("#money_customerEDUCATION_form").form("validate")) {
            focusTab('assets');
            $("#money_customerEDUCATION_form").form("validate");
            return false;
        }
        ...
        var policyInfoListVO = new Array();// 保单贷信息
        $("#money_customer_POLICY_div").find("form").each(function(ind,formObj){
            policyInfoListVO.push($(formObj).serializeObject());
        });

        var carInfoVO = $("#money_customerCar_form").serializeObject();// 车辆信息
        var providentInfoVO = $("#money_customerProvident_form").serializeObject();// 公积金信息
        var insuranceVO = $("#money_customerSOCIAL_INSURANCE_form").serializeObject();// 社保信息
        var cardLoanInfoVO = $("#money_customerCardLoan_form").serializeObject();// 卡友贷信息
        var estateInfoVO = $("#money_customerHome_form").serializeObject();// 房产信息
        var merchantInfoVO = $("#money_customerMerchant_form").serializeObject();// 淘宝账户信息
        var educationInfoVO = $("#money_customerEDUCATION_form").serializeObject();// 学历信息
        if (isNotNull(carInfoVO.carBuyDate)) {// 购买时间
            carInfoVO.carBuyDate = carInfoVO.carBuyDate + "-01";
        }
        if (isNotNull(carInfoVO.carLoanIssueDate)) {// 车贷发放年月
            carInfoVO.carLoanIssueDate = carInfoVO.carLoanIssueDate + "-01";
        }
        if (isNotNull(estateInfoVO.estateBuyDate)) {// 购房时间
            estateInfoVO.estateBuyDate = estateInfoVO.estateBuyDate + "-01";
        }
        if (isNotNull(estateInfoVO.estateLoanIssueDate)) {//房贷发放年月
        ...
        if (isNotNull(educationInfoVO.graduationDate)) {// 毕业时间
            educationInfoVO.graduationDate = educationInfoVO.graduationDate + "-01";
        }
        ...
        var assetsInfo = new Object();
        assetsInfo.policyInfo = policyInfoListVO; // 保单信息
        assetsInfo.carInfo = carInfoVO;// 车辆信息
        assetsInfo.fundInfo = providentInfoVO;// 公积金信息
        assetsInfo.socialInsuranceInfo = insuranceVO;// 社保信息
        assetsInfo.cardLoanInfo = cardLoanInfoVO;//卡友贷信息
        assetsInfo.estateInfo = estateInfoVO;// 房产信息
        assetsInfo.masterLoanInfo = merchantInfoVO;// 淘宝账户信息
        assetsInfo.educationInfo = educationInfoVO; // 学历信息
        ...
        auditAmendEntryVO.assetsInfo = assetsInfo;
...
function moneyCustomerAddContacts(contactType) {
    if ($("#money_contactInfoVOList_div").find("form").length>=10) {
    ...
    if (isNotNull(contactType) && "00013" == contactType) {// 标识添加配偶信息
        ...
        contactsHTML.find("input[name='contactName']").textbox({// 加载联系人姓名
            ...
            validType: ["peopleNameWithParam['联系人姓名']"]
        });
        contactsHTML.find("input[name='contactEmpName']").textbox({// 加载单位名称
            width: 415,
            validType: ["notAllNumberAndLength[200, '单位名称']"],
...
function initCustomerInfo() {
        ...
        onChange: function (newValue, oldValue) {
         vm.applyInfo.applicantInfo.workInfo.cusWorkType = newValue;
            if (newValue == "00001") {           // 私营业主
                ...
                $("#money_customerPrivateOwner_div").show();                            // 显示私营业主信息
                ...
                $("#money_customerPrivateOwner_div").hide();                            // 隐藏私营业主信息
    ...
    $(".peopleName").each(function (ind, val) {
        $(val).textbox({
            required: true,
            validType: isRelation(ind) ? "peopleNameWithParam['配偶姓名']" : "peopleNameWithParam['联系人姓名']",
    ...
    $("#money_customerInfo_carInfo_carBuyPrice").numberbox({
        ...
        precision: 0,
        buttonText: '元',
        missingMessage: "请填购买价格!",
        validType: ["nonnegativePnum['购买价格']", "range[0, 99999999, '购买价格']"]
    });
    ...
    $("#money_carLoanOrg").textbox({
        ...
        validType: ["notAllNumberAndLength[200, '车贷发放机构']"]
    });
    ...
    $("#money_carLoanInsuranceCompany").textbox({
        ...
        validType: ["notAllNumberAndLength[200, '交强险承保公司']"]
    });
    ...
    // 还款日
    $("#money_cardLoanInfo_dueDate").numberbox({
        required: true,
        precision: 0,
        missingMessage: "请填写还款日!",
        validType: ["nonnegativePnum['还款日']", "range[1, 31, '还款日']"]
    });
    // ------------------------------------------------ end  客户卡友贷信息初始化-------------------------------------------------------------------
    ...
    $("#money_customerInfo_estateInfo_estateLoan").combobox({
        ...
        onChange: function (newValue, oldValue) {
            // 申请类型为TOPUP和RELOAN类型不对资产信息进行校验
            var applyType = vm.applyInfo.baseInfo.applyType;
            if (isNotNull(applyType) && applyType != 'TOPUP' && applyType != 'RELOAN') {
    ...
    $("#money_education_schoolName").textbox({
        ...
        validType: ["notAllNumberAndLength[200, '院校名称']"]
    });
